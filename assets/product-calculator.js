(function () {
  ...
  function extractServices(q) {
    ...
    if (q.frete) {
      var f = q.frete;
      if (f.valor_pac)            items.push({ name: 'PAC',            price: f.valor_pac,            days: f.prazo_pac });
      if (f.valor_sedex)          items.push({ name: 'SEDEX',          price: f.valor_sedex,          days: f.prazo_sedex });
      if (f.valor_jadlog_package) items.push({ name: 'Jadlog Package', price: f.valor_jadlog_package, days: f.prazo_jadlog_package });
      if (f.valor_jadlog_dotcom)  items.push({ name: 'Jadlog .com',    price: f.valor_jadlog_dotcom,  days: f.prazo_jadlog_dotcom });
    }

    if (q.servicos && Array.isArray(q.servicos)) {
      q.servicos.forEach(function (s) {
        if (s.valor) items.push({ name: s.servico || 'Frete', price: s.valor, days: s.prazo });
      });
    }

    if (items.length === 0) {
      if (q.valorpac)   items.push({ name: 'PAC',   price: q.valorpac,   days: q.prazopac });
      if (q.valorsedex)  items.push({ name: 'SEDEX', price: q.valorsedex, days: q.prazosedex });
    }
